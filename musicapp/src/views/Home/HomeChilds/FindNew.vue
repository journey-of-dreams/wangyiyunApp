<template>
  <div class="findNew">
    <nav-bar>
      <template #center>
        <div class="searchInput">
          <i class="iconfont icon-sousuo"></i> 大家都在搜水星记
        </div>
      </template>
      <template #right>
    ...
    <van-swipe :autoplay="3000" indicator-color="white">
      <van-swipe-item v-for="item in homeData.swiperData" :key="item.bannerId">
        <img :src="item.pic" />
    ...
    <ul class="dragonBall">
      <li v-for="item in homeData.dragonBall" :key="item.id">
      ...
      <ul>
        <li class="listItem">
          <van-swipe vertical :show-indicators="false" :autoplay="3000">
            <van-swipe-item
              v-for="item in homeData.commendSwiper"
              :key="item.id"
            >
        ...
        <li
          class="listItem"
          v-for="item in homeData.commendList"
          :key="item.id"
        >
          <img :src="item.picUrl" alt="" />
          <p class="name">{{ item.name }}</p>
